...
import me.kw.mall.dao.FavoriteDao;
  ...
  public static class FavoriteHelper {
    ...
    public void onRecvMsg(MessageData msg) {
      if (msg.valiateReq(FavoriteService.IsProductFavoritedRequest.class)) {
        CommonReturn response = JsonSerializerFactory.Create().decode(
            new String(msg.getContext().data()), CommonReturn.class);

        if (null != response) {
          if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
            if (isQueryFavoriteState) {
              if (null != tvewFavorite) {
                CommonUtil.setDrawableTop(mContext, tvewFavorite, resIdFavorited);
              }
            } else {

              IsFavorited data = JsonSerializerFactory.Create()
                  .decode(new String(msg.getContext().data()), IsFavorited.class);
              delProductFavorite(mDataLoader, Integer.parseInt(data.data));
            }
          } else {
            if (isQueryFavoriteState) {
              if (null != tvewFavorite) {
                CommonUtil.setDrawableTop(mContext, tvewFavorite, resIdUnFavorite);
              }
            } else {
              if (productId > 0) {
        ...
        if (null != response) {
          if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
            ShowMsg.showToast(mContext.getApplicationContext(), "添加收藏成功");
            CommonUtil.setDrawableTop(mContext, tvewFavorite, resIdFavorited);
            BaseApplication.getInstance().setActivityResult(ResultActivity.CODE_EDIT_FAVORITE, null);
          } else {
            ShowMsg.showToast(mContext.getApplicationContext(), response.message);
          }
        } else {
          ShowMsg.showToast(mContext.getApplicationContext(), "添加收藏失败");
        }
      } else if (msg.valiateReq(FavoriteService.DelProductFavoriteRequest.class)) {
        CommonReturn response = msg.getRspObject();
        if (null != response) {
          if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
            ShowMsg.showToast(mContext.getApplicationContext(), "删除收藏成功");
            CommonUtil.setDrawableTop(mContext, tvewFavorite, resIdUnFavorite);
            BaseApplication.getInstance().setActivityResult(ResultActivity.CODE_EDIT_FAVORITE, null);
          } else {
            ShowMsg.showToast(mContext.getApplicationContext(), response.message);
          ...
          ShowMsg.showToast(mContext.getApplicationContext(), "删除收藏失败");
        }
        ...
        if (null != response) {
          if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
            ShowMsg.showToast(mContext.getApplicationContext(), "取消收藏成功");
            tvewFavorite.setText("收藏店铺");
            BaseApplication.getInstance().setActivityResult(ResultActivity.CODE_EDIT_FAVORITE, null);
          } else {
            ShowMsg.showToast(mContext.getApplicationContext(), response.message);
          }
        ...
        CommonReturn response =
            JsonSerializerFactory.Create().decode(new String(msg.getContext().data()),
                CommonReturn.class);
        ...
        if (null != response) {
          if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
            if (isQueryFavoriteState) {
              ...
              IsFavorited data = JsonSerializerFactory
                      .Create()
                      .decode(new String(msg.getContext().data()), IsFavorited.class);
              delShopFavorite(mDataLoader, Integer.parseInt(data.data));
            }
          } else {
            if (isQueryFavoriteState) {
              if (null != tvewFavorite) {
