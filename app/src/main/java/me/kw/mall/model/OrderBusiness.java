...
import me.kw.mall.dao.OrderDao;
...
//            arrayCIds[i] = listCIds.get(i);
  ...
  public static boolean queryRefundOrder(Context context,
                                         HttpDataLoader httpDataLoader, String type, String
                                             orderId, String productId,
                                         BigDecimal money, String reason, String explain,
                                         String[] coverIds) {
   ...
   */
  public static void showAddressInfo(Order order, TextView tvewPersonal,
                                     TextView tvewPhone, TextView tvewAddress) {
    if (null == order) {
      return;
    }
  ...
  public static void showOrderItem(Context context,
    ...
    for (int i = 0; i < orderItems.length; i++) {
      View view = LayoutInflater.from(context).inflate(R.layout.layout_shop_order_item, null);
      OrderItem orderItem = orderItems[i];
      ImageView imgvewPhoto = (ImageView) view.findViewById(R.id.imgvew_photo_show);
      imageLoader.displayImage(Endpoint.HOST + orderItem.image_path, imgvewPhoto, options);
...
//            tvewOrderComplete.setVisibility(View.VISIBLE);
  ...
  public static void showOrderStateAction(Order order, TextView tvewOrderReceiving,
                                          TextView tvewOrderReceiver,
                                          TextView tvewOrderComplete,
                                          TextView tvewOrderSubmit,
                                          TextView tvewOrderCancel, TextView tvewOrderDel,
                                          TextView tvewOrderReview) {
    tvewOrderComplete.setVisibility(View.GONE);
    tvewOrderCancel.setVisibility(View.GONE);
    tvewOrderReview.setVisibility(View.GONE);
    ...
    if ("0".equals(order.order_status)) {
      // 未提交
      tvewOrderCancel.setVisibility(View.VISIBLE);
      tvewOrderSubmit.setVisibility(View.VISIBLE);
    } else if ("3".equals(order.order_status) || "交易取消".equals(order.status_text)) {
      //交易取消
      tvewOrderComplete.setVisibility(View.VISIBLE);
      tvewOrderDel.setVisibility(View.VISIBLE);
    } else if (("1".equals(order.order_status) && "0".equals(order.shipping_status)) || "待发货"
        .equals(order.status_text)) {
      // 待发货
      tvewOrderComplete.setVisibility(View.VISIBLE);
      tvewOrderReceiving.setVisibility(View.VISIBLE);
    } else if ("待收货".equals(order.status_text)) {
  ...
  public static boolean isAllOrderItemRefunded(Order order) {
