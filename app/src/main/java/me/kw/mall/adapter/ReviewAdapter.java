...
public class ReviewAdapter extends CommonAdapter {
  ...
  public ReviewAdapter(Context context, List<?> list) {
    super(context, list);
    options = ImageLoaderUtil.getDisplayImageOptions(R.drawable.img_empty_logo_small);
  }
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    CircleImageView imgvewHeadPicture = findViewById(convertView, R.id.imgvew_head_picture_show);
    ...
    ImageView imgvewPhoto4 = findViewById(convertView, R.id.imgvew_photo4_show);
    ...
    mImageLoader.displayImage(Endpoint.HOST + review.avatar, imgvewHeadPicture, options);
    ...
    if (null != review.images && review.images.length > 0) {
      ...
      for (int i = 0; i < review.images.length; i++) {
        if (i == 0 && !TextUtils.isEmpty(review.images[i])) {
          ...
          imgvewPhoto1.setOnClickListener(new ImageClickListener(i, review.images));
