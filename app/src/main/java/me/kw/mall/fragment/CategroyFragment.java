...
import me.kw.mall.activity.ProductActivity;
import me.kw.mall.adapter.Category1Adapter;
...
public class CategroyFragment extends MallBaseFragment
    implements BaseFragment.OnFragmentCreatedListener, ExpandableListView.OnChildClickListener {

  @Bind(R.id.tvew_category_name) TextView mTvewCategoryName;
  ...
  private AdCategoryFragment mAdCgFragment;
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    ProductBusiness.queryAdHomeIndex(getHttpDataLoader(), "category_header");
  ...
  @Override protected void initUI() {
    ...
    mListView.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() {
      @Override public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition,
                                            long id) {
    ...
    mAdCgFragment = new AdCategoryFragment();
...
//    }
  ...
  public void onCategory2ItemClick(int group, int position) {
    ...
    if (null != child && child.size() > position) {
      ...
      bundle.putString("child", JsonSerializerFactory.Create().encode(mlistCategory.get(group)
          ._child.get(position)._child));
      bundle.putString("title", categoryName);
      bundle.putString("category_id", categoryId);
      bundle.putString("type", mProductType);
      getIntentHandle().intentToActivity(bundle, ProductActivity.class);
    }
  }

  @OnClick(R.id.rlayout_search_click)
...
//            selectCategory1(mGroupId);
   ...
   * 在这里定义请求对象试试
   */
  @HttpPortal(path = "http://jzb.jxlnxx.com/?s=/Api/Ad/getList", method = Context.Method.Post,
