...
import me.kw.mall.adapter.ShopOrderAdapter;
...
import service.api.Payment;
import service.entity.OrderService;
import service.entity.PayService;

public class ShopOrderFragment extends MallBaseFragment implements
    PullToRefreshView.OnHeaderRefreshListener, PullToRefreshView.OnFooterRefreshListener,
    ShopOrderAdapter.CallBackInteface {
  ...
  public void onRecvMsg(MessageData msg) throws Exception {
    if (msg.valiateReq(OrderService.QueryOrderRequest.class)
      ...
      OrderPageResult response = msg.getRspObject();
      if (CommonValidate.validateQueryState(getActivity(), msg, response)) {
        ...
        if (null == mAdapter) {
          mAdapter = new ShopOrderAdapter(getActivity(), mPageInditor.getAll());
      ...
      if (null != response) {
        if (response.code == ErrorCode.INT_QUERY_DATA_SUCCESS) {
          ...
          OrderBusiness.queryOrderDetail(getHttpDataLoader(), mPageInditor.getSelectItem()
              .order_id);
        } else {
          dismissWaitDialog();
