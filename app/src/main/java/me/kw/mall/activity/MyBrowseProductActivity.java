...
@ZTitleMore(false)
public class MyBrowseProductActivity extends MallBaseActivity implements
  ...
  @Override public void onRecvMsg(MessageData msg) throws Exception {
    if (msg.valiateReq(FavoriteService.BrowseProductsRequest.class)) {
      ...
      ProductFavoritePageResult response = msg.getRspObject();
      if (CommonValidate.validateQueryState(this, msg, response)) {
        ...
        if (mPageInditor.getAll().size() == 0) {
          ...
          getDataEmptyView().showViewDataEmpty(false, true, msg, "还没有浏览过任何商品");
        }
      }
  ...
  private void refreshData(boolean isPullRefresh) {
    ...
    FavoriteBusiness.queryProductBrowse(getHttpDataLoader(), mPageInditor.getPageNum());
  }
  ...
  public void onClickDelete(int position) {
    FavoriteBusiness.delProductBrowse(getHttpDataLoader(), Integer.parseInt(mPageInditor.get
        (position).id));
  ...
  public void onClickSelect(int position) {
    ProductBusiness.intentToProductDetailActivity(this, null, Integer.parseInt(mPageInditor.get
        (position).product_id));
  }
