...
import me.kw.mall.enumerate.Security7days;
import me.kw.mall.enumerate.SecurityDelivery;
...
public abstract class AddProductActivity extends BaseActivity {
  ...
  protected List<UploadFile.FileInfo> mImageFile = new ArrayList<UploadFile.FileInfo>();
  ...
  public void setClickListener() {
    ...
    if (null != shipAddress) {
      shipAddress.setOnClickListener(new View.OnClickListener() {
    ...
    if (null != mTvewAddPhoto) {
      mTvewAddPhoto.setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View v) {
    ...
    if (null != mImgvewPhoto1) {
      mImgvewPhoto1.setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View v) {
          miImagePosition = 0;
          isEditImage = true;
          onClickTvewSelectPhoto();
    ...
    if (null != mImgvewPhoto2) {
      mImgvewPhoto2.setOnClickListener(new View.OnClickListener() {
    ...
    if (null != mImgvewPhoto3) {
      mImgvewPhoto3.setOnClickListener(new View.OnClickListener() {
    ...
    if (null != mRlayoutVideo) {
      mRlayoutVideo.setOnClickListener(new View.OnClickListener() {
  ...
  protected void showProductInfo(ProductItem productItem) {
    if (null != productItem) {
      ...
      if (null != productItem.movie) {
        ...
        mVideoFile = new UploadFile.FileInfo();
        ...
        if (!TextUtils.isEmpty(productItem.movie.thumb_image_path)) {
          ...
          new VideoPlayView(this).getVideoThumbnail(videoUrl,
              new VideoPlayView.ThumbnailCallBack() {
  ...
  public void onRecvMsg(MessageData msg) throws Exception {
    ...
    } else if (msg.valiateReq(FileService.PostFileRequest.class)) {
    ...
    } else if (msg.valiateReq(FileService.PostFileVideoRequest.class)) {
      UploadFile uploadFile = ProviderProductBusiness.parseUploadFile(this, msg);
      if (null != uploadFile && null != uploadFile.Data) {
  ...
  private void showCategory() {
    if (null == mPopupWindow) {
      ...
      mPopupWindow.setOnItemChangeListener(new WheelViewPopupWindow.OnItemChangeListener() {

        @Override public void onPopItemChange(int firstPosition,
  ...
  protected void showTimePopupWindow(final TextView textView) {
    ...
    mTimePopupWindow.setOnTimeEnsureListener(new TimePopupWindow.OnTimeEnsureListener() {
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (resultCode == Activity.RESULT_OK) {
      ...
      if (requestCode == PictureSelectUtil.REQ_CODE_RECORD) {
        mstrFilePath = mPictureSelectUtil.getVideo(requestCode, resultCode, data);
        if (!StringUtil.isEmptyString(mstrFilePath)) {
          Bitmap bitmap = ThumbnailUtils.createVideoThumbnail(mstrFilePath, MediaStore.Images
          ...
          if (null != bitmap) {
            mstrThumbnailPath = BitmapUtil.saveBitmap(this, bitmap, token + ".jpg", false);
            sendPostVideoRequest(mstrFilePath);
          }
        }
      } else {
        mstrFilePath = mPictureSelectUtil.getPicture(requestCode, resultCode, data);
        if (!StringUtil.isEmptyString(mstrFilePath)) {
